...
-- DROP FUNCTION IF EXISTS hex_grid2_width(widthkm FLOAT, xmin FLOAT, ymin FLOAT, xmax FLOAT, ymax FLOAT, inputsrid INTEGER, workingsrid INTEGER, ouputsrid INTEGER, OUT gid TEXT, OUT geom geometry);
CREATE OR REPLACE FUNCTION hex_grid_width(widthkm FLOAT, xmin FLOAT, ymin FLOAT, xmax FLOAT, ymax FLOAT, inputsrid INTEGER, workingsrid INTEGER, ouputsrid INTEGER, OUT gid TEXT, OUT hex geometry)
...
BEGIN
  ...
  RETURN QUERY (
    SELECT DISTINCT sqt.* FROM (
      SELECT pre || x_series::TEXT || y_series::TEXT AS gid, ST_Transform(ST_SetSRID(ST_Translate(geom, x_series::FLOAT, y_series::FLOAT), workingsrid), ouputsrid) AS hex
        FROM generate_series(x1, x2, (qtrwidth * 6)) AS x_series,
             generate_series(y1, y2, (halfheight * 2)) AS y_series,
             (
               SELECT '1' AS pre, ST_GeomFromText(
     ...
     INNER JOIN public.aus_buffer AS aus
     ON ST_Intersects(aus.geom, sqt.hex)
  );
